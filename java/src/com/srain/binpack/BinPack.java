...
public class BinPack {
	...
	private static void doPack(OutputStream out, Object obj, String charsetName) throws java.io.IOException {
		if (obj == null) {
			packNull(out);
			...
			packString(out, "XXX-not-pack-unsupported-type-" + obj.getClass().getName(), "UTF-8");
		}
	}
	...
	private static void packMap(OutputStream out, Map map, String charsetName) throws IOException {
		...
		while (it.hasNext()) {
			...
			doPack(out, entry.getKey(), charsetName);
			doPack(out, entry.getValue(), charsetName);
		}
		out.write(BIN_TAG_SHUT);
	}
